...
For hosted database migrations that introduce a dedicated function owner:

- [ ] Probe `current_user` and `session_user`; do not assume the dashboard or
      connection-pool role is the migration executor
- [ ] Account for both sides of `ALTER ... OWNER`: the migrator needs `SET` on
      the new owner and the new owner needs temporary `CREATE` on the schema
- [ ] Revoke temporary membership and schema creation privileges in the final
      migration, then assert `SET`/`INHERIT` options rather than merely checking
      whether a PostgreSQL 17 creator ADMIN row exists
- [ ] Compile every function from a fresh database on the deployed PostgreSQL
      major; static SQL text scans cannot parse PL/pgSQL bodies
